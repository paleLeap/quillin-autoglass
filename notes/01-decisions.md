...
- 2026-09-19 - The phone menu is a MENU, not a strip. Tapping the mark grows it
  8%, dims the rest of the page behind a scrim, and stacks the categories
  vertically under it. Choosing one opens that page and the whole thing fades
  away. Tapping anywhere off the mark dismisses it. The header does not change
  height at any point (108px throughout) because the list is absolutely
  positioned.
  The scrim is body::after rather than an element: it is a state of the page,
  not a thing on it, and it needs no markup to reason about.
- 2026-09-19 - CACHE BUSTING, finally. css and js URLs in index.html now carry a
  content hash, regenerated by ./bump.sh. This is not housekeeping: a cached
